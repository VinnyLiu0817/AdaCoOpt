...
class CollectStatistics:
    def __init__(self, results_file_name=os.path.dirname(__file__)+'/results.csv', is_single_run=False):
        ...
        if is_single_run:
            with open(results_file_name, 'a') as f:
                f.write(
                    'case,tValue,lossValue,predictionAccuracy,betaAdapt,deltaAdapt,rhoAdapt,cAdapt,tau,it_each_local,it_each_global\n')
                f.close()
        else:
            with open(results_file_name, 'a') as f:
                f.write(
    ...
    def collect_stat_end_local_round(self, case, tau, it_each_local, it_each_global, control_alg, model, train_image,
                                     train_label, test_image, test_label, w_global, total_time_recomputed, global_loss, current_k):
        ...
        if control_alg is not None:
            ...
            if isinstance(control_alg, ControlAlgAdaptiveTauServer):
                ...
                if use_dynamic_tau is False and control_alg.c_adapt_mvaverage is not None:
                    self.c_adapts.append(control_alg.c_adapt_mvaverage)
                elif self.is_single_run:
                    self.c_adapts.append(np.nan)
        ...
        if self.is_single_run:
            loss_value = 'NaN'
            prediction_accuracy = 'NaN'
            if current_k % evaluate_gap == 0:
                if evaluate_train_loss_per_round is True:
                    if batch_train_loss is True:
                        loss_value = global_loss
                    else:
                        loss_value = model.loss(train_image, train_label, w_global)
                    self.loss_values.append(loss_value)
                    print("***** lossValue(Train): " + str(loss_value))
                if evaluate_acc_per_round is True:
                    prediction_accuracy = model.accuracy(test_image, test_label, w_global)
                    self.prediction_accuracies.append(prediction_accuracy)
                    print("***** Accuracy(Test): " + str(prediction_accuracy))
            ...
            with open(self.results_file_name, 'a') as f:
                f.write(str(case) + ',' + str(total_time_recomputed) + ',' + str(loss_value) + ','
                        ...
                        + ',' + str(self.c_adapts[-1]) + ',' + str(tau) + ',' + str(it_each_local) + ',' + str(it_each_global) + '\n')
                f.close()
    ...
    def collect_stat_end_global_round(self, sim, case, tau_setup, total_time, model, train_image, train_label,
        ...
        # with open(self.results_file_name, 'a') as f:
        #     f.write('Summary' + ',' + 'Total time:' + ',' + str(total_time) + ','
        #             + 'Loss value:' + ','
        #             + str(loss_final) + ',' + 'Accuracy:' + ',' + str(accuracy_final) + '\n')
        #     f.close()
