...
from config import SERVER_ADDR, SERVER_PORT, dataset_file_path, stream_mode, samp_method, use_dynamic_tau
if use_dynamic_tau is False:
    ...
    from control_algorithm.default_adaptive_tau import ControlAlgAdaptiveTauClient, ControlAlgAdaptiveTauServer
...
try:
    while True:
        ...
        use_min_loss = msg[10]
        sim = msg[11]
        init_buffer = msg[12]  # initialize the number of the data sample in the buffer
        buff_size = msg[13] # Define the buffer size for each client
        ...
        if batch_size >= total_data:
            ...
            if stream_mode is True:
                sampler.init_buffer(init_buffer, buff_size)
            else:
                sampler.init_buffer(50000, 50000)
        ...
        while True:
            ...
            current_bs = msg[5]
            data_arrive = msg[6] # The number of the new-arrived data
            print('Current batch size:', current_bs)
            if data_arrive > 0:
                if samp_method == 'res':
                    sampler.buffer_res_update(data_arrive)
                elif samp_method == 'ran':
                    sampler.buffer_random_update(data_arrive)
                elif samp_method == 'fifo':
                    sampler.buffer_fifo_update(data_arrive)
                else:
                    raise ValueError("Invalid sampling method!")
                    # sampler.buffer_update(data_arrive)
            ...
            print("Current flow idx: ",sampler.get_flow_idx())
            # print("Current buffer: ", sampler.get_buffer())
            ...
            for i in range(0, tau_config):
                ...
                if batch_size < total_data:
                    ...
                    if (not isinstance(control_alg, ControlAlgAdaptiveTauClient)) or (i != 0) or (train_indices is None) \
                        ...
                        sample_indices = sampler.get_next_batch(current_bs)
